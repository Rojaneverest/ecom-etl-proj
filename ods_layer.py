...
def create_external_stage(conn):
    """Creates an external stage pointing to the S3 bucket."""
    try:
        with conn.cursor() as cursor:
...
def copy_data_from_s3_to_snowflake(conn, s3_key, table_name):
    """
    Uses Snowflake's COPY INTO command to load data directly from S3.
    This is much faster than downloading and using write_pandas.
    """
    ods_table_name = table_name.upper() # Use a distinct variable name for the ODS table
    ...
    try:
        with conn.cursor() as cursor:
            ...
            # Perform the COPY INTO operation
            copy_sql = f"""
            COPY INTO "{ods_table_name}"
            ...
            logger.info(f"Executing COPY INTO command for '{ods_table_name}'...")
            ...
            # ... (rest of the row parsing logic remains the same)
            for row in cursor:
...
def validate_data_load(conn, table_name):
    """Validates the data load by checking row count."""
    try:
        with conn.cursor() as cursor:
            # Explicitly set context for this operation
            cursor.execute(f"USE DATABASE {SNOWFLAKE_DATABASE}")
...
def main():
    """Main function to run the S3 to Snowflake ODS data ingestion pipeline using COPY INTO."""
    logger.info("--- Starting Optimized ODS Ingestion Script for Snowflake (COPY INTO) ---")
    ...
    try:
        ...
        # --- 4. Process and Load Each Dataset using COPY INTO ---
        total_rows_loaded = 0
        successful_loads = 0
        ...
        for s3_dataset_name, s3_key in latest_parquet_objects.items():
            ...
            try:
                # Use COPY INTO to load data directly from S3
    ...
    logger.info("--- Optimized ODS Ingestion Script Finished ---")
